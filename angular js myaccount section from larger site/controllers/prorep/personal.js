...
app.controller('ProrepPersonalEnrolles', ['$rootScope', '$scope', '$http', '$filter', '$timeout', '$state', function ($rootScope, $scope, $http, $filter, $timeout, $state) {
	console.log("ProReps Personal Enrolles Controller file personal.js");
	...
	$scope.cnf = {
		field: 'GV',
		type: $state.is('app.enrollees') ? 'P' : 'T',
		hold: []
	};
	...
	$scope.$watch('data.list.enroll', function(newVal, oldVal) {
        if (newVal == oldVal) {
        }
	...
	$scope.loadTree = function() {
		...
		$scope.cnf.hold = [];
		$http.post('/api/prorep/personalenrollees', {id:$scope.searchmember,token:token,type:$scope.cnf.type})
	...
	$scope.parseData = function(data) {
		var field = {FirstName:'',LastName:'',LoginId:'',MemberId:parseInt,SubType:'',Email:'',Telephone:'',NextDate:'',Rank:$scope.rank,'Smart-Ship':'',GV:parseFloat,PV:parseFloat,TV:parseFloat};
		var that = this;
		...
		this.child = function(child, level){
			...
			d.level = level;
			chart += '<li>';
			chart += '<div class="TreeDiv"><b>'+d.FirstName+' '+d.LastName+'</b><br />'+(500000+d.MemberId)+'<br />'+$filter('currency')(d.PV)+'/'+$filter('currency')(d[$scope.cnf.field])+'<p><span>'+(d.PV >= 200 ? '<i class="fa fa-money text-success"></i>' : '')+(d['Smart-Ship'] ? '<i class="fa fa-truck text-warning"></i>' : '')+'</span><a class="'+d.MemberId+'">Downline</a></p></div>';
			var tree = {label: d.FirstName+' '+d.LastName+' '+(d.LoginId ? '('+d.LoginId+') ':'')+(500000+d.MemberId)+' '+$filter('currency')(d.PV)+'/'+$filter('currency')(d[$scope.cnf.field]),data:d,children: []};
			if (level !== 0) list.push(d);
			if (child.members) {
				chart += '<ul>';
		...
		return {tree:[this.child(data[0][$scope.searchmember])],list: {data:list,total:total}, chart: chart, count: count};
